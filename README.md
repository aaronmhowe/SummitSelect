...
- I do not believe the results are entirely accurate. When reversing the data from their normalized scores, Sandia Peak being ranked #1 doesn't seem entirely accurate.
    ...
    - My thinking is that the normalization has unintentionally created an extreme bias when computing the weighted sum, as resorts with relatively different values in run count, have been normalized to perhaps be too close to being equal, thus cannot be ranked properly. However, I am still chopping away at this minor issue, so I cannot say accurately as of now if this is the problem. Overall, the technical goals of this project have been achieved.
